...
export default function NewsPage() {
    return (
        <main className="min-h-screen bg-background-light text-slate-900 transition-colors duration-200">
            {/* Hero Section */}
            ...
            <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                    {articles.map((article) => (
                        <Link href={`/news/${article.slug}`} key={article.id} className="group flex flex-col h-full bg-white border border-transparent hover:border-red-500 transition-all duration-300 shadow-sm hover:shadow-md overflow-hidden rounded-sm">
                            ...
                            <div className="p-6 flex flex-col flex-grow">
                                <div className="text-xs text-slate-500 font-bold mb-3 uppercase tracking-wide">
                                    {article.date}
                                </div>
